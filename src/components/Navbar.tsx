...
const Navbar = () => {
  ...
  const toggleMobileProductsMenu = useCallback(() => {
    ...
    >
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20 relative">
          ...
          {/* Desktop Navigation - Extracted component */}
          <DesktopNav 
            user={user}
            isActive={isActive}
  ...
  return (
    <div className="hidden md:flex items-center space-x-4">
      ...
      <div className="relative group">
        <button 
          className="relative px-4 py-2.5 rounded-xl font-semibold text-blue-600 hover:text-blue-800 transition-all duration-500 group flex items-center gap-2"
          aria-haspopup="menu"
          aria-expanded="false"
        >
      ...
      {['about', 'contact'].map((item) => (
        <NavLink 
          ...
          label={item === 'contact' ? 'Contact Us' : 'About Us'}
        />
      ))}
...
const NavLink = ({ href, isActive, children, label }: NavLinkProps) => {
  return (
    <Link 
      to={href}
...
const UserMenu = ({ user, showUserMenu, setShowUserMenu, handleLogout, userMenuRef }: UserMenuProps) => {
  if (!user) {
    return (
  ...
  return (
    <div className="relative pl-4 border-l border-blue-200/30" ref={userMenuRef}>
      <button 
      ...
      {showUserMenu && (
        <div 
          className="absolute top-full right-0 mt-3 w-64 bg-gradient-to-b from-white/95 to-white/90 backdrop-blur-xl shadow-2xl rounded-2xl py-3 z-50 border border-white/50"
          role="menu"
          ...
          <div className="rounded-xl border border-blue-200/30 bg-gradient-to-b from-blue-50/20 to-transparent p-1">
            ...
            >
              <div className="flex items-center gap-3">
                <ChevronDown 
                  size={16} 
                  className={`transition-transform duration-500 ${isMobileProductsOpen ? 'rotate-180' : ''}`} 
                />
                <span>Products</span>
              </div>
...
const MobileNavLink = ({ href, isActive, onClick, children }: MobileNavLinkProps) => {
  return (
    <Link 
      to={href} 
...
const MobileDropdownLink = ({ href, onClick, children }: MobileDropdownLinkProps) => {
  return (
    <Link 
      to={href}
